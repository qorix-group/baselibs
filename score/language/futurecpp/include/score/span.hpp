...
public:
    ...
    /// \brief Constructs a span that is a view over \p range; the resulting span has size() == score::cpp::ssize(range) and
    ...
    span(Range&& range) : data_{score::cpp::data(range)}, size_{score::cpp::ssize(range)}
    {
    }
    ...
    /// count is -1, the number of elements in the subspan is `size() - offset` (i.e., it ends at the end of *this.).
    ///
    /// \param offset skipped elements at the front
    /// \param count number of elements in the resulting subspan, unless `count` is -1, in which case the resulting
    /// subspan contains all remaining elements. \pre `0 <= offset <= size()` \pre `-1 <= count <= size() - offset`
    /// \return The requested subspan `r`, such that `r.data() == this->data() + offset`. If `count` is -1,
    /// `r.size() == this->size() - offset`; otherwise `r.size() == count`.
    span subspan(const size_type offset, const size_type count = -1) const
    {
        SCORE_LANGUAGE_FUTURECPP_PRECONDITION_DBG(offset >= 0);
        SCORE_LANGUAGE_FUTURECPP_PRECONDITION_DBG(count >= -1);
        SCORE_LANGUAGE_FUTURECPP_PRECONDITION_DBG(offset <= size());
        SCORE_LANGUAGE_FUTURECPP_PRECONDITION_DBG(count <= (size() - offset));
        return (count == -1) ? span{data() + offset, size() - offset} : span{data() + offset, count};
    }
    ...
    {
        SCORE_LANGUAGE_FUTURECPP_PRECONDITION_DBG(count >= 0);
...
{
    static_assert(sizeof(T) < static_cast<std::size_t>(std::numeric_limits<std::ptrdiff_t>::max()), "Overflow.");
    const std::ptrdiff_t size_bytes{view.size() * static_cast<std::ptrdiff_t>(sizeof(T))};
