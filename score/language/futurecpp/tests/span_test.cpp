...
{
    std::int32_t data[]{23, 42, 72};
    span<const std::int32_t> view{&data[0], ssize(data)};
...
TEST(span, CopyConstruction)
{
    ...
    EXPECT_EQ(view2.size(), score::cpp::ssize(data));
...
{
    {
        ...
        const span<const std::int32_t> view{data.data(), ssize(data)};
        EXPECT_TRUE(view.empty());
    }
    {
...
TEST(span, subspan)
{
    ...
    const span<const std::int32_t> view{data.data(), ssize(data)};
    {
        ...
        EXPECT_TRUE(std::equal(unit.begin(), unit.end(), data.begin() + offset, data.begin() + offset + count));
    ...
    }
    {
        ...
        EXPECT_TRUE(std::equal(unit.begin(), unit.end(), data.begin() + offset, data.end()));
        EXPECT_TRUE(unit.data() == view.data() + offset);
        EXPECT_TRUE(unit.size() == view.size() - offset);
    }
    {
        std::ptrdiff_t offset{0};
        std::ptrdiff_t count{view.size()};
        const span<const std::int32_t> unit{view.subspan(offset, count)};
...
/// @testmethods TM_REQUIREMENT
/// @requirement CB-#9338069
TEST(span, WhenSubspanAndOffsetIsNegativeThenViolated)
{
    const std::vector<std::int32_t> data{12, 5, -5, -6};
    const span<const std::int32_t> view{data.data(), ssize(data)};

    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.subspan(-1, 0));
}

/// @testmethods TM_REQUIREMENT
/// @requirement CB-#9338069
TEST(span, WhenSubspanAndCountIsLessThanMinusOneThenViolated)
{
    const std::vector<std::int32_t> data{12, 5, -5, -6};
    const span<const std::int32_t> view{data.data(), ssize(data)};

    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.subspan(0, -2));
}
...
TEST(span, WhenSubspanAndOffsetIsGreaterThanSizeThenViolated)
{
    ...
    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.subspan(5, 0));
}
...
TEST(span, WhenSubspanAndOffsetPlusCountIsGreaterThanSizeThenViolated)
{
    ...
    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.subspan(0, 5));
    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.subspan(1, 4));
    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.subspan(4, 1));
}
...
TEST(span, first)
{
    ...
    const span<const std::int32_t> view{data.data(), ssize(data)};
    {
        ...
        EXPECT_TRUE(std::equal(unit.begin(), unit.end(), data.begin(), data.begin() + count));
...
TEST(span, WhenNonConstRangeIsPassedToNonConstSpan)
{
    ...
    EXPECT_EQ(view.size(), score::cpp::ssize(data));
...
/// @testmethods TM_REQUIREMENT
/// @requirement CB-#9338069
TEST(span, WhenFirstAndCountIsNegativeThenViolated)
{
    const std::vector<std::int32_t> data{12, 5, -5, -6};
    const span<const std::int32_t> view{data.data(), ssize(data)};

    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.first(-1));
}
...
/// @testmethods TM_REQUIREMENT
/// @requirement CB-#9338069
TEST(span, WhenLastAndCountIsNegativeThenViolated)
{
    const std::vector<std::int32_t> data{12, 5, -5, -6};
    const span<const std::int32_t> view{data.data(), ssize(data)};

    SCORE_LANGUAGE_FUTURECPP_EXPECT_CONTRACT_VIOLATED(view.last(-1));
}
...
TYPED_TEST(span_associated_type_test, that_type_traits_api_is_compatible_with_std20_span)
{
    ...
    static_assert(std::is_same<typename GivenSpan::size_type, std::ptrdiff_t>::value, "");
...
template <typename T>
struct span_associated_type_test_v1 : testing::Test
{
};

using span_element_types = testing::Types<double, const double>;
TYPED_TEST_SUITE(span_associated_type_test_v1, span_element_types, /*unused*/);

/// @testmethods TM_REQUIREMENT
/// @requirement CB-#9338069
TYPED_TEST(span_associated_type_test_v1, that_type_traits_api_is_compatible_with_std20_span)
{
    using ElementType = TypeParam;
    using GivenSpan = span<ElementType>;
    using ExpectedValueType = std::remove_cv_t<ElementType>;
