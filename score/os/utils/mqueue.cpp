...
class MQueue::MQueuePrivate
{
    ...
    score::cpp::expected_blank<score::os::Error> open() noexcept;
...
MQueue::MQueuePrivate::MQueuePrivate(const size_t id) : m_id(id), m_name(create_name(m_id))
{
...
MQueue::MQueuePrivate::MQueuePrivate(const std::string& name,
                                     const AccessMode mode,
                                     const size_t max_msg_size,
                                     const size_t max_msgs)
...
MQueue::MQueuePrivate::MQueuePrivate(const std::string& name) noexcept
...
score::cpp::expected_blank<score::os::Error> MQueue::MQueuePrivate::open_create(const size_t max_msg_size,
                                                                                const size_t max_msgs,
                                                                                const AccessMode mode) noexcept
{
    /* False positive, as UL is a suffix, not a macro */
    /* LCOV_EXCL_BR_START: Unable to run the false assert case */
    assert(max_msg_size > 1UL);
    assert(max_msgs > 1UL);
    /* LCOV_EXCL_BR_STOP */
    ...
    const score::os::Mqueue::ModeFlag perm{
        ...
        score::os::Mqueue::ModeFlag::kReadOthers};
    ...
    {
        flags = flags | score::os::Mqueue::OpenFlag::kNonBlocking;
    }
...
void MQueue::MQueuePrivate::send(const std::string& msg) const
{
    score::cpp::ignore = send(msg.c_str(), msg.length());
}

/* Methods MQueue::MQueuePrivate::send and MQueue::send belong to different classes and can easily be differentiated */
score::cpp::expected_blank<score::os::Error> MQueue::MQueuePrivate::send(const char* const msg,
...
ssize_t MQueue::MQueuePrivate::timed_send(const char* const msg,
                                          const size_t length,
                                          const std::chrono::milliseconds timeout) const noexcept
{
    timespec times = score::common::timeout_in_timespec(timeout, std::chrono::system_clock::now());
 ...
 * differentiated */
score::cpp::expected<size_t, Error> MQueue::MQueuePrivate::receive(char* const msg) const noexcept
{
    /* Methods MQueue::MQueuePrivate::receive and MQueue::receive belong to different classes and can easily be
     * differentiated */
    const auto ret =
