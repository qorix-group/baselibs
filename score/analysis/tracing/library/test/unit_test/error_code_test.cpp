...
TEST(ERROR_CODE, MessageForMatchWithErrorCode)
{
    ...
    ASSERT_TRUE(generic_trace_api_error_domain
                    .MessageFor(static_cast<score::result::ErrorCode>(ErrorCode::kDaemonIsDisconnectedRecoverable))
    ...
    ASSERT_TRUE(generic_trace_api_error_domain
                    .MessageFor(static_cast<score::result::ErrorCode>(ErrorCode::kFailedToProcessJobsFatal))
                    .compare("Failed to process Jobs") == 0);
}
...
std::vector<std::tuple<ErrorCode, bool>> test_cases{
    ...
    std::make_tuple(ErrorCode::kDaemonIsDisconnectedRecoverable, true),
    // Failed to process jobs (Fatal)
    std::make_tuple(ErrorCode::kFailedToProcessJobsFatal, false),
    // Terminal (Fatal)
